"""v156: Universal foundation model — train on N-1 cohorts (across
ALL 5 cohorts: 4 gliomas + PROTEAS-brain-mets), evaluate on the
held-out 1 cohort. 5-fold leave-one-cohort-out (LOCO).

The 'foundation model' framing: a single model trained on all
available data deployed across all institutions and diseases.

For each held-out cohort c:
  Train: union of all OTHER cohorts (4 cohorts total)
  Test:  c (the held-out cohort)

This generalises v141 (UCSF train, glioma LOCO) and v152 (4 gliomas
train, PROTEAS test) into a unified LOCO across all 5 cohorts.

Outputs:
    Nature_project/05_results/v156_universal_foundation_loco.json
"""
...
OUT_JSON = RESULTS / "v156_universal_foundation_loco.json"
OUT_CSV = RESULTS / "v156_universal_foundation_per_patient.csv"
...
def load_proteas():
    rows = []
    with tempfile.TemporaryDirectory(prefix="proteas_v156_") as td:
        work = Path(td)
...
def main():
    print("=" * 78, flush=True)
    print("v156 UNIVERSAL FOUNDATION MODEL (5-fold leave-one-cohort-out)", flush=True)
    ...
    print(f"\n=== FINAL UNIVERSAL FOUNDATION MODEL SUMMARY (5-fold LOCO) ===",
          flush=True)
    print(f"  {'Held-out':<22s} {'N':>4s} {'learn-out':>10s} {'bim-out':>10s} "
    ...
    for cohort in ALL_COHORTS:
        ...
        print(f"  {cohort:<22s} {s['n']:>4d} "
              f"{s['learned_outgrowth_pct']:>9.2f}% {s['bimodal_outgrowth_pct']:>9.2f}% "
              f"{s['ensemble_outgrowth_pct']:>9.2f}% {s['ensemble_overall_pct']:>9.2f}%",
              flush=True)

    # Cohort-mean
    metrics = ["learned_outgrowth_pct", "bimodal_outgrowth_pct",
    ...
    out = {"version": "v156",
           "experiment": "Universal foundation model 5-fold LOCO across all 5 cohorts",
