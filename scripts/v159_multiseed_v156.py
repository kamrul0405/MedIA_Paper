"""v159: Multi-seed v156 universal foundation model 5-fold LOCO.

Replicates v156 across 3 seeds (42, 123, 999). Reports mean +/- SE
per held-out cohort. Required rigour for top-tier review of the
v156 flagship.

Outputs:
    Nature_project/05_results/v159_multiseed_v156.json
"""
...
OUT_JSON = RESULTS / "v159_multiseed_v156.json"
OUT_CSV = RESULTS / "v159_multiseed_v156_per_patient.csv"
...
def load_proteas():
    rows = []
    with tempfile.TemporaryDirectory(prefix="proteas_v159_") as td:
        work = Path(td)
...
def main():
    print("=" * 78, flush=True)
    print("v159 MULTI-SEED v156 UNIVERSAL FOUNDATION MODEL", flush=True)
    print(f"  cohorts: {ALL_COHORTS}", flush=True)
    print(f"  seeds: {SEEDS}; device: {DEVICE}; epochs={EPOCHS}", flush=True)
    print("=" * 78, flush=True)
    ...
    print(f"\n=== MULTI-SEED AGGREGATE (mean +/- SE across {len(SEEDS)} seeds) ===",
          flush=True)
    out = {"version": "v159",
           "experiment": "Multi-seed v156 universal foundation 5-fold LOCO",
    ...
    for cohort in ALL_COHORTS:
        ...
        for m in metrics:
            ...
            agg[f"{m}_min"] = float(min(vals))
            agg[f"{m}_max"] = float(max(vals))
            print(f"  {cohort:<22s} {m:<22s} {agg[f'{m}_mean']:>7.2f}% "
                  f"{agg[f'{m}_se']:>5.2f} {agg[f'{m}_min']:>7.2f}% "
                  f"{agg[f'{m}_max']:>7.2f}%", flush=True)
    ...
    for m in metrics:
        ...
        cohort_mean_summary[m] = {
            "mean": float(np.mean(vals)),
            "se": float(np.std(vals, ddof=1) / np.sqrt(len(SEEDS))),
            "min": float(min(vals)), "max": float(max(vals)),
            "per_seed": {s: cohort_mean_per_seed[s][m] for s in SEEDS},
        }
        print(f"  {m:<25s}: {cohort_mean_summary[m]['mean']:5.2f}% +/- "
              f"{cohort_mean_summary[m]['se']:.2f} "
              f"(range [{cohort_mean_summary[m]['min']:.2f}, "
              f"{cohort_mean_summary[m]['max']:.2f}])", flush=True)
    out["cohort_mean_summary"] = cohort_mean_summary
    out["v156_single_seed_baseline_pct"] = {
        "ensemble_outgrowth": 80.34,
        "ensemble_overall": 89.10,
    }
