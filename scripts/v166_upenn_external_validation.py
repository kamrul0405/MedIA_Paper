"""v166: UPENN-GBM TRUE external validation (6th cohort).

Train universal foundation model on ALL 5 cohorts (UCSF + MU + RHUH +
LUMIERE + PROTEAS-brain-mets; n=635) and evaluate on UPENN-GBM
(n=41) — a TRULY external cohort that was NEVER used in training
or LOCO across the entire session.

UPENN-GBM was the tier-3 sensitivity cohort with manually-segmented
baseline + FLAIR-based pseudo-followup masks, processed only as 2D
cropped (48 x 48) slices. We tile to 3D (16, 48, 48) by replicating
the 2D slice along the depth axis (creates a thick-slab volume
compatible with the (16, 48, 48) trained foundation model).

This is the cleanest cross-cohort cross-disease cross-pseudolabel
generalisation test in the entire session.

Outputs:
    Nature_project/05_results/v166_upenn_external_validation.json
"""
...
OUT_JSON = RESULTS / "v166_upenn_external_validation.json"
OUT_CSV = RESULTS / "v166_upenn_external_per_patient.csv"
...
def load_proteas():
    rows = []
    with tempfile.TemporaryDirectory(prefix="proteas_v166_") as td:
        work = Path(td)
...
def load_upenn():
    """Load UPENN-GBM 2D cropped masks and tile to 3D (16, 48, 48)."""
    if not UPENN_NPZ.exists():
        print(f"  UPENN npz not found at {UPENN_NPZ}", flush=True)
        return []
    data = np.load(UPENN_NPZ, allow_pickle=True)
    pids = data["pids"]
    base48 = data["base48"]  # (41, 48, 48)
    rec48 = data["rec48"]    # (41, 48, 48)
    rows = []
    for i, pid in enumerate(pids):
        # Tile 2D mask along depth axis to make 3D (16, 48, 48)
        m_2d = (base48[i] > 0).astype(np.float32)
        t_2d = (rec48[i] > 0).astype(np.float32)
        m_3d = np.tile(m_2d[None, :, :], (16, 1, 1))  # (16, 48, 48)
...
def train_unet(train_data, seed, epochs=EPOCHS, batch_size=4):
    ...
    for ep in range(epochs):
        ...
        if (ep + 1) % 5 == 0:
            print(f"    epoch {ep+1}/{epochs}: loss = {float(loss.item()):.4f}",
                  flush=True)
    return model
...
def main():
    print("=" * 78, flush=True)
    print("v166 UPENN-GBM TRUE EXTERNAL VALIDATION (6th cohort, never trained)",
          flush=True)
    print(f"  Train: ALL 5 cohorts {ALL_COHORTS} (n=~635)", flush=True)
    print(f"  Test:  UPENN-GBM (n=41 patients, 2D->3D tiled)", flush=True)
    ...
    for cohort in ALL_COHORTS:
        ...
        print(f"  {cohort}: {len(rows)} patients/follow-ups", flush=True)
    print(f"  Total training: {len(train_data)} patients/follow-ups", flush=True)

    print(f"\nLoading UPENN-GBM external test cohort...", flush=True)
    upenn_data = load_upenn()
    print(f"  UPENN-GBM: {len(upenn_data)} patients (2D->3D tiled to "
          f"{TARGET_SHAPE})", flush=True)

    if not upenn_data:
        print("  No UPENN data available; aborting", flush=True)
        return

    print(f"\n=== Training universal foundation model on all 5 cohorts ===",
          flush=True)
    t0 = time.time()
    model = train_unet(train_data, SEED)
    print(f"  training done in {time.time()-t0:.0f}s", flush=True)

    print(f"\n=== Evaluating on UPENN-GBM (n={len(upenn_data)}; TRULY external) ===",
          flush=True)
    ...
    print(f"  learned:  overall {learned_ovr:5.2f}%  outgrowth {learned_out:5.2f}%",
          flush=True)
    print(f"  bimodal:  overall {bim_ovr:5.2f}%  outgrowth {bim_out:5.2f}%",
          flush=True)
    print(f"  ensemble: overall {ens_ovr:5.2f}%  outgrowth {ens_out:5.2f}%",
          flush=True)

    print(f"\n=== UPENN external vs v156 5-cohort LOCO mean ===", flush=True)
    print(f"  v156 5-cohort cohort-mean ens-out: 80.34%", flush=True)
    print(f"  v159 multi-seed cohort-mean ens-out: 77.58% +/- 1.63", flush=True)
    print(f"  v166 UPENN external (tiled 2D->3D): {ens_out:.2f}% ens-out", flush=True)
    ...
    out = {"version": "v166",
           "experiment": "UPENN-GBM TRUE external validation (6th cohort)",
           "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
           "training_cohorts": ALL_COHORTS,
           "n_train": len(train_data),
           "test_cohort": "UPENN-GBM",
           "n_test": len(rows),
           "limitation": ("UPENN-GBM is processed as 2D 48x48 cropped masks; "
                            "tiled to 3D (16,48,48) along depth axis"),
           "results_upenn": {
           ...
           "comparison_to_loco_5cohort": {
               "v156_5cohort_mean_ens_out_pct": 80.34,
               "v159_multiseed_cohort_mean_ens_out_pct": 77.58,
               "v166_upenn_external_ens_out_pct": ens_out,
           }}
