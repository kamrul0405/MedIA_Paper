"""v172: Few-shot UPENN-GBM adaptation curve.

Trains the universal foundation model on ALL 5 cohorts (UCSF + MU +
RHUH + LUMIERE + PROTEAS-brain-mets; n=635), then fine-tunes on
N in {0, 5, 10, 20, 41} UPENN-GBM patients with leave-rest-out
evaluation. Plots ensemble outgrowth vs N to map the practical
minimum-local-data threshold for deploying the foundation model
to a new institution.

For each N:
  - Sample N UPENN patients as 'fine-tuning set'
  - Fine-tune the pretrained foundation model for 10 epochs on the N
    UPENN patients
  - Evaluate on the remaining 41 - N UPENN patients
  - Record cohort-mean ensemble outgrowth + per-patient values

Uses the same data setup as v166 (UPENN tiled 2D->3D).

Outputs:
    Nature_project/05_results/v172_few_shot_upenn.json
"""
...
OUT_JSON = RESULTS / "v172_few_shot_upenn.json"
OUT_CSV = RESULTS / "v172_few_shot_upenn_per_patient.csv"
...
def load_proteas():
    rows = []
    with tempfile.TemporaryDirectory(prefix="proteas_v172_") as td:
        work = Path(td)
...
def train(model, train_data, seed, epochs, lr, batch_size=4, label="pretrain"):
    ...
    for ep in range(epochs):
        ...
        if (ep + 1) % 5 == 0 or ep == epochs - 1:
            print(f"    [{label}] epoch {ep+1}/{epochs}: loss = "
...
def main():
    print("=" * 78, flush=True)
    print("v172 FEW-SHOT UPENN ADAPTATION CURVE", flush=True)
    print(f"  Pretrain: 5 cohorts (n=~635); fine-tune on N UPENN patients", flush=True)
    print(f"  N grid: {N_GRID}", flush=True)
    print(f"  device: {DEVICE}", flush=True)
    print("=" * 78, flush=True)

    print("\nLoading 5 training cohorts...", flush=True)
    train_data = []
    ...
    print(f"  Total training: {len(train_data)}", flush=True)

    print(f"\nLoading UPENN-GBM...", flush=True)
    upenn_data = load_upenn()
    print(f"  UPENN: {len(upenn_data)} patients", flush=True)

    # Step 1: pretrain foundation model on 5 cohorts
    print(f"\n=== Step 1: Pretrain foundation model on 5 cohorts (~3 min) ===",
          flush=True)
    t0 = time.time()
    pretrained_model = UNet3D(in_ch=2, base=24).to(DEVICE)
    train(pretrained_model, train_data, SEED, EPOCHS_PRETRAIN, LR_PRETRAIN,
          label="pretrain")
    print(f"  pretrain done in {time.time()-t0:.0f}s", flush=True)
    pretrained_state = deepcopy(pretrained_model.state_dict())

    # Pre-shuffle UPENN once for reproducibility
    rng = np.random.default_rng(17201)
    upenn_shuffled = list(upenn_data)
    rng.shuffle(upenn_shuffled)

    all_results = []
    summary = {}
    for n in N_GRID:
        t0 = time.time()
        print(f"\n=== Step 2: Few-shot N={n} ===", flush=True)
        finetune_set = upenn_shuffled[:n]
        test_set = upenn_shuffled[n:]
        if len(test_set) == 0:
            # If N=41, evaluate on all UPENN (no held-out)
            test_set = upenn_shuffled
        # Reload pretrained weights (clean slate per N)
        ft_model = UNet3D(in_ch=2, base=24).to(DEVICE)
        ft_model.load_state_dict(pretrained_state)
        if n > 0:
            train(ft_model, finetune_set, SEED, EPOCHS_FINETUNE, LR_FINETUNE,
                  label=f"finetune-N{n}")
        rows = evaluate(ft_model, test_set)
        for r in rows:
            r["N_finetune"] = n
        ...
        if rows:
            ...
            ens_out = ens_ovr = learned_out = bim_out = float("nan")
        ...
        print(f"  N={n} (test n={len(test_set)}): "
              f"learned-out {learned_out:.2f}% bim-out {bim_out:.2f}% "
              f"ens-out {ens_out:.2f}% ens-ovr {ens_ovr:.2f}% "
              f"({time.time()-t0:.0f}s)", flush=True)
        del ft_model; torch.cuda.empty_cache(); gc.collect()
    ...
    # Summary
    print(f"\n=== Few-shot adaptation curve on UPENN ===", flush=True)
    print(f"  {'N_finetune':<12s} {'N_test':<8s} {'learned-out':>12s} "
          f"{'bim-out':>10s} {'ens-out':>10s} {'ens-ovr':>10s}", flush=True)
    for n in N_GRID:
        s = summary[n]
        print(f"  {n:<12d} {s['n_test']:<8d} {s['learned_outgrowth_pct']:>11.2f}% "
              f"{s['bimodal_outgrowth_pct']:>9.2f}% "
              f"{s['ensemble_outgrowth_pct']:>9.2f}% "
              f"{s['ensemble_overall_pct']:>9.2f}%", flush=True)

    out = {"version": "v172",
           "experiment": "Few-shot UPENN-GBM adaptation curve",
           "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
           "pretrain_cohorts": ALL_COHORTS,
           "n_pretrain": len(train_data),
           "test_cohort": "UPENN-GBM",
           "n_upenn": len(upenn_data),
